...
export class DiscountListComponent {
  ...
  dataSourceSignal = computed(() =>
    this.discountListSignal()?.content?.map((it: IDiscount) => {
  ...
  delete = (discount: IDiscount): void => {
    ...
    const content = this.translateService.instant('DISCOUNT.DELETED.CONTENT', {
      name: discount.name,
    });
  ...
  sentToUsers = (discount: IDiscountAll): void => {
    ...
    executeDialog(
      ...
      (result) => {
        if (result) {
          this.discountStore.sendToCustomers(
            result.discountId,
            result.customerIds,
          );
        }
