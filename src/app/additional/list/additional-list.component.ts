...
export class AdditionalListComponent {
  ...
  dataSourceSignal = computed(() =>
    this.additionalListSignal()?.content?.map((additional: IAdditional) => {
  ...
  resultsLengthSignal = computed(
    () => this.additionalListSignal()?.totalElements || 0,
  );
  ...
  delete = (additional: IAdditional): void => {
    ...
    executeDialogNoWidth(
      this.dialog,
      DialogComponent,
      { title, content, value: additional, variant: 'warning' },
      (result) => {
        if (result) {
