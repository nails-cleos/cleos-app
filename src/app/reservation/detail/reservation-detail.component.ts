...
export class ReservationDetailComponent {
  ...
  private static readonly PROFESSIONAL_PAYMENT_TYPES = [
    'CASH',
    'TRANSFER',
    'MOLLIE',
  ];
  ...
  readonly paymentsLoading = computed(
    () => !!this.reservationSignal() && this.paymentStore.isLoading(),
  );
  readonly historyLoading = computed(
    () => !!this.reservationSignal() && this.historiesSignal() === undefined,
  );
  ...
  displayedColumns: string[] = this.historyTableColumns.map(
    (column) => column.key,
  );
  ...
  paymentDisplayedColumns: string[] = this.paymentTableColumns.map(
    (column) => column.key,
  );
  ...
  readonly paymentsValue = toSignal(
    this.payments.valueChanges.pipe(startWith(this.payments.getRawValue())),
  );
  ...
  private getReservationPaymentOptions = (
    ...
    const options = this.paymentOptions().filter((option) =>
      roomPaymentTypes.has(option.type),
    );
    ...
    if (isReservationAdmin) {
      const professionalPaymentTypes = new Set(
        ReservationDetailComponent.PROFESSIONAL_PAYMENT_TYPES,
      );
      return options.filter((option) =>
        professionalPaymentTypes.has(option.type),
      );
    }
  ...
  isEditablePaymentType = (paymentType?: string): boolean => {
    return (
      !!this.professionalId() &&
      this.switchablePaymentTypes.includes(paymentType || '')
    );
  };

  getAllowedPaymentTypes = (paymentType?: string): string[] => {
    return this.isEditablePaymentType(paymentType)
      ? this.switchablePaymentTypes
      : [];
  };

  getStatusBadgeColor = (state?: string): string =>
    findStateColor(state || 'DEFAULT', this.isDarkMode());
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (reservation) {
        ...
        this.options.set(
          this.getReservationPaymentOptions(
            reservation,
            isCustomer,
            this.isReservationAdmin,
          ),
        );
        ...
        if (isProfessionalAdmin) {
          ...
          if (reservation.state === 'APPROVED') {
            const discount = this.createAction(
              this.translateService.instant(
                'RESERVATION.TREATMENT.DISCOUNT.FIELD',
              ),
              ReservationIconName.discount,
              'discount',
            );
        ...
        const note = this.createAction(
          this.translateService.instant('RESERVATION.NOTE.FIELD'),
          ReservationIconName.note,
          'note',
        );
        const noteTransaction = ReservationDetailComponent.createTransaction(
          'note',
          (): void => this.addNote(),
        );
        ReservationDetailComponent.addTransitionToAllStates(
        ...
        const overview = this.createAction(
          this.translateService.instant('COMMON.BUTTON.VIEW'),
          ReservationIconName.overview,
          'overview',
        );
        const overviewTransaction =
          ReservationDetailComponent.createTransaction('overview', (): void =>
            this.overview(),
          );
        ReservationDetailComponent.addTransitionToAllStates(
          'overview',
    ...
    effect(() => {
      ...
      if (currentIndex >= 0 && currentIndex < allReservations.length - 1) {
        ...
        const nextTransition = ReservationDetailComponent.createTransaction(
          next,
          () => {
            this.hasFetched.set(false);
            this.navigationService.navigate([
  ...
  overview() {
    if (this.isCustomer()) {
      ...
      this.navigationService.navigate([
        'users',
        this.reservationSignal()?.customer?.id,
        'overview',
      ]);
    }
  }
  ...
  addDiscount() {
    ...
    if (reservation) {
      executeDialog(
        ...
        (result) => {
          if (result) {
            this.reservationStore.updateDiscount(
              reservation.id,
              result.discountId,
            );
          }
  ...
  private static addTransitionToAllStates = (
    ...
    for (const state in ReservationDetailComponent.stateMachineDefinition) {
      if (state !== 'initialState') {
        if (
          Object.hasOwn(
            ReservationDetailComponent.stateMachineDefinition,
            state,
          )
        ) {
          const stateDefinition =
            ReservationDetailComponent.stateMachineDefinition[state];
  ...
  private static getDateTimeDetail = (reservation: IReservationAll): Date =>
    newDateTimestamp(reservation.timestamp);
  ...
  showTimeZone = (reservation?: IUpcomingAll): boolean =>
    !isSameTimeZone((reservation || this.reservationSignal())?.room.timeZone);
  ...
  onChangeState = (id: string): void => {
    ...
    const title = this.translateService.instant(
      'RESERVATION.CHANGE_STATE.TITLE',
    );
    const action = this.translateService.instant(
      `RESERVATION.CHANGE_STATE.ACTION.${id.toUpperCase()}`,
    );
    const content = this.translateService.instant(
      'RESERVATION.CHANGE_STATE.CONTENT',
      { action },
    );
  ...
  private professionalMachine = (self: this): any => {
    ...
    const approve = this.createAction(
      translateService.instant('RESERVATION.ACTION.APPROVE'),
      ReservationIconName.approved,
      'approve',
    );
    const start = this.createAction(
      translateService.instant('RESERVATION.ACTION.START'),
      ReservationIconName.started,
      'start',
    );
    const complete = this.createAction(
      translateService.instant('RESERVATION.ACTION.COMPLETE'),
      ReservationIconName.completed,
      'complete',
    );
    const edit = this.createAction(
      translateService.instant('RESERVATION.ACTION.EDIT'),
      ReservationIconName.edit,
      'edit',
    );
    const cancel = this.createAction(
      translateService.instant('RESERVATION.ACTION.CANCEL'),
      ReservationIconName.cancelled,
      'cancel',
    );
    const book = this.createAction(
      translateService.instant('RESERVATION.ACTION.BOOK'),
      ReservationIconName.book,
      'book',
    );
    const sendMessage = this.createAction(
      translateService.instant('RESERVATION.ACTION.SEND'),
      ReservationIconName.send,
      'send',
    );
    const coffeeMessage = this.createAction(
      translateService.instant('RESERVATION.ACTION.COFFEE'),
      ReservationIconName.coffee,
      'coffee',
    );
    const change = this.createAction(
      translateService.instant('RESERVATION.ACTION.CHANGE'),
      ReservationIconName.change,
      'change',
    );

    const more = this.createAction(
      translateService.instant('RESERVATION.ACTION.MORE'),
      ReservationIconName.more,
      'more',
    );

    const color = this.createAction(
      translateService.instant('RESERVATION.ACTION.COLOR'),
      ReservationIconName.color,
      'color',
    );

    const clone = this.createAction(
      translateService.instant('RESERVATION.ACTION.CLONE'),
      ReservationIconName.clone,
      'clone',
    );
    ...
    const approveTransaction = ReservationDetailComponent.createTransaction(
      'approved',
      (): void => {
    ...
    const sendMessageTransaction = ReservationDetailComponent.createTransaction(
      'send',
      (): void => {
        if (startDate) {
          let key;
          let date = getTime(startDate, self.language);
          let treatment = ReservationDetailComponent.createBullet(
            reservation.treatment.name,
          );
          treatment += reservation.additional?.map((additional) =>
            ReservationDetailComponent.createBullet(additional.name),
          );
          switch (true) {
          ...
          const message = translateService.instant(`WHATSAPP.SEND.${key}`, {
            date,
            treatment,
          });
          window.open(
    ...
    const coffeeMessageTransaction =
      ReservationDetailComponent.createTransaction('send', (): void => {
    ...
    const startTransaction = ReservationDetailComponent.createTransaction(
      'started',
      (): void => {
    ...
    const editTransaction = ReservationDetailComponent.createTransaction(
      'edited',
      (): void => {
        self.navigationService.navigate(['reservation', id, 'edit'], {
          state: { roomId },
        });
      },
    );

    const completeTransaction = ReservationDetailComponent.createTransaction(
      'completed',
      (): void => {
    ...
    const moreTransaction = ReservationDetailComponent.createTransaction(
      'more',
      (): void => {
    ...
    const changeCustomerTransaction =
      ReservationDetailComponent.createTransaction('change', (): void =>
        self.changeUser(reservation),
      );

    const changeColorTransaction = ReservationDetailComponent.createTransaction(
      'color',
      (): void => self.changeColor(reservation),
    );

    const cloneTransaction = ReservationDetailComponent.createTransaction(
      'clone',
    ...
    const cancelTransaction = ReservationDetailComponent.createTransaction(
      'cancelled',
      (): void =>
    ...
    const finishTransaction = ReservationDetailComponent.createTransaction(
      'completed',
      (): void => {
    ...
    const bookTransaction = ReservationDetailComponent.createTransaction(
      'booked',
      (): void => {
        const customerId = reservation.customer.id;
        const roomId = reservation.room.id;
  ...
  private customerMachine = (self: this): any => {
    ...
    if (reservation.paymentRequired) {
      ...
      const paymentPending = paymentPaid?.filter(
        (p: IPayment) => p.status === 'PENDING',
      )[0];
      if (paymentPending) {
        const notify = this.createAction(
          translateService.instant('RESERVATION.ACTION.NOTIFY'),
          ReservationIconName.notify,
          'notify',
        );
        ...
        const pay = this.createAction(
          translateService.instant('RESERVATION.ACTION.PAY'),
          ReservationIconName.payment,
          'pay',
        );
          ...
          ReservationDetailComponent.createTransaction(
            ...
            (): void => {
              this.navigationService.navigate([
                'me',
                'payment',
                paymentPaid?.filter((p: IPayment) => p.status !== 'APPROVED')[0]
                  ?.id,
              ]);
            },
          );
    ...
    if (price.total > price.totalPaid) {
      ...
      const transaction = ReservationDetailComponent.createTransaction(
        'paid',
        (): void => {
          this.navigationService.navigate([
            'me',
  ...
  private addActions = (): void => {
    if (!this.paymentTableColumns.some((column) => column.key === 'actions')) {
      this.paymentTableColumns.splice(this.paymentTableColumns.length - 1, 0, {
        key: 'actions',
        hideOnMobile: true,
      });
      this.paymentDisplayedColumns = this.paymentTableColumns.map(
        (column) => column.key,
      );
    }
  };
  ...
  private changeUser = (reservation: IReservationAll): void =>
    executeDialog(
      ...
      (result) => {
        if (result) {
          this.reservationStore.updateCustomer(
            reservation.id,
            result.customerId,
          );
        }
