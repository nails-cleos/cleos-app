...
export class ReservationCalendarService {
  ...
  getRoomSchedule(room: IRoomAll): RoomSchedule {
    ...
    const { monday, tuesday, wednesday, thursday, friday, saturday, sunday, exclude } = availability;
    const { min, max } = getStartEndDay(monday, tuesday, wednesday, thursday, friday, saturday, sunday, room.timeZone);
