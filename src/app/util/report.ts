        ...
        if (income.total.payments.length) {
          ...
          income.total.payments.forEach((payment: ISummaryTotal) => {
            ...
            const row = worksheet.addRow(
              [++totalRow, link,
                exportFormatDate(newDateTimestamp(income.timestamp, timeZone), DEFAULT_LOCALE, timeZone),
          ...
          const row = worksheet.addRow(
            [++totalRow, link, exportFormatDate(newDateTimestamp(income.timestamp, timeZone), DEFAULT_LOCALE, timeZone),
              income.customerName, income.description, income.total.discountDescription, income.color, typeValue,
...
const monthlyExpenseWorksheet = (
  ...
  const headers = ['DATE', 'INVOICE', 'SUPPLY_STORE', 'DESCRIPTION', 'TYPE', 'SUB_TYPE', 'GROSS', 'NET', 'BTW'];
  createHeader(worksheet, headers, translate);
  setSubtitle(worksheet, cells, cellNumber);
    ...
    if (rowData.length) {
      rowData.forEach((expense) => {
        ...
        expense.total.payments.forEach((payment: ISummaryTotal) => {
          ...
          const row = worksheet.addRow(
            [++totalRow, link,
              exportFormatDate(newDateTimestamp(expense.timestamp, timeZone), DEFAULT_LOCALE, timeZone),
...
const getSaleRowData = (saleSummary: IMonthlySummarySale[], timeZone: string, env: EnvService): {
  ...
  const rowData = saleSummary.map((sale, index) => {
    ...
    return [index + 1, id, exportFormatDate(newDateTimestamp(sale.timestamp, timeZone), DEFAULT_LOCALE, timeZone),
...
const getExpenseRowData = (expenseSummary: IMonthlySummaryExpense[], timeZone: string, env: EnvService): {
  ...
  const rowData = expenseSummary.map((expense, index) => {
    ...
    return [index + 1, id,
      exportFormatDate(createNewDateZonedTime(expense.timestamp, timeZone), DEFAULT_LOCALE, timeZone),
