...
export const resetTheme = (
  overlayContainer: Pick<OverlayContainer, 'getContainerElement'>,
  cookieService: Pick<CookieService, 'get' | 'set'>,
  themeService: Pick<ThemeService, 'setColorschemesOptions'>,
  theme?: Theme,
  cssClass?: string,
): string => {
...
export const getStateOrder = (state?: string): number =>
  eventState().find((color) => color.name === state)?.order || 100;
