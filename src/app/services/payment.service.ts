...
export class PaymentService {
  ...
  getPayment = (id: string): Observable<IPaymentAll | undefined> =>
    this.http.get<IPaymentAll>(toUrl(this.urlV1, id));

  getPaymentOptions = (): Observable<IPaymentOption[]> =>
    this.http.get<IPaymentOption[]>(toUrl(this.urlV1, 'options'), {
      ...skipLoadingOverlay(),
    });
  ...
  add = (
    ...
    this.http.post<IPay>(
      toUrl(this.getKey(path), id, this.url, status),
      paymentStatus,
    );
  ...
  createPaymentLinkByReservationId = (
    ...
    this.http.post<IPayment>(
      toUrl(this.urlV1, 'reservations', reservationId),
      payment,
    );

  adjustPayments = (payment: IPaymentRequest[]): Observable<void> =>
    this.http.patch<void>(this.urlV1, payment);

  updatePayment = (
    id: string,
    payment: IReservationPayment,
  ): Observable<IApiResponse> =>
    this.http.patch<IApiResponse>(toUrl(this.urlV1, id), payment);

  recreate = (id: string, paymentType: string): Observable<IPayment> =>
    this.http.patch<IPayment>(
  ...
  getPaymentByResourceId = (
    id: string,
    path: 'reservation' | 'transaction',
  ): Observable<IPaymentResource> =>
    this.http.get<IPaymentResource>(
  ...
  public notifyPayment = (
    ...
    this.http.patch<IPay>(toUrl(this.getKey(path), resourceId, this.url, id), {
      preferenceId,
      paymentType,
    });
