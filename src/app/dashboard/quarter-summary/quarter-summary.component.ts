...
import { dateMonthYear, getDateQuarter, getNowTimeZone } from '@app/util/dates';
...
export class QuarterSummaryComponent {
  ...
  form: FormGroup<QuarterSummaryForm> =
    this.formBuilder.group<QuarterSummaryForm>({
      selectedRoom: this.formBuilder.control(undefined),
      selectedQuarter: this.formBuilder.control<number>(
        getDateQuarter(this.now),
      ),
      date: this.formBuilder.control<Date>(this.now),
    });

  private dateSignal = toSignal(this.getForm.date.valueChanges);
  private selectedQuarterSignal = toSignal(
    this.getForm.selectedQuarter.valueChanges,
  );
    ...
    effect(() => {
      ...
      if (quarterSummaryMapValue) {
        if (quarterSummaryMapValue.size === 1) {
          ...
          if (
            quarterSummaryMapValue.size > 1 &&
            allElementsHaveSameKeyFilterValue(quarterSummaryMapValue, [
              'currency',
              'id',
            ])
          ) {
      ...
      if (room) {
        ...
        this.monthSummaries()?.forEach((value) => {
          value.total.forEach((t) => {
            switch (t.type) {
              case 'INCOME':
                newQuarterSummaryTotals.income = new Total(
                  newQuarterSummaryTotals.income.gross + t.gross,
                  newQuarterSummaryTotals.income.btw + t.btw,
                  newQuarterSummaryTotals.income.net + t.net,
                );
                break;
              case 'EXPENSE':
                newQuarterSummaryTotals.expense = new Total(
                  newQuarterSummaryTotals.expense.gross + t.gross,
                  newQuarterSummaryTotals.expense.btw + t.btw,
                  newQuarterSummaryTotals.expense.net + t.net,
                );
                break;
              case 'CASH':
                newQuarterSummaryTotals.cash = new Total(
                  newQuarterSummaryTotals.cash.gross + t.gross,
                  newQuarterSummaryTotals.cash.btw + t.btw,
                  newQuarterSummaryTotals.cash.net + t.net,
                );
                break;
            }
          });
          totals = new Total(
            totals.gross + value.totalGross,
            totals.btw + value.totalBTW,
            totals.net + value.totalNet,
          );
          totalsWithoutCash = new Total(
            totalsWithoutCash.gross + value.totalWithoutGross,
            totalsWithoutCash.btw + value.totalWithoutBTW,
        ...
        this.quarterSummaryTotals.set(
          new SummaryTotals(
            newQuarterSummaryTotals.income,
            newQuarterSummaryTotals.expense,
            newQuarterSummaryTotals.cash,
            totalsWithoutCash,
            totals,
          ),
        );
  ...
  get quarterSummaryMap():
    Map<ISummaryRoom, { monthSummaries: IMonthSummary[] }> | undefined {
  ...
  goBack(): void {
    this.navigationService.navigate(['dashboard', 'year', 'summary'], {
  ...
  exportQuarterSummary = (): void => {
    ...
    if (monthSummariesValue?.length) {
      const now = getNowTimeZone();
      const quarter =
        this.selectedQuarterSignal() || getDateQuarter(getNowTimeZone());
      const year = this.year() || now.getFullYear();
      const workbook = createQuarterSummary(
        quarter,
        year,
        monthSummariesValue,
  ...
  private getAllMonthSummaries = (
    quarterSummaries: IMonthSummary[],
    result: IMonthSummary[],
  ): IMonthSummary[] => {
    return result.map((m) => {
