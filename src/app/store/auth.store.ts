...
const setPersistedSnapshot = (store: any) => queueMicrotask(() => {
...
export const AuthStore = signalStore(
  ...
  withMethods((
    ...
    return {
      ...
      getDriveToken(): void {
        firebaseService.signInWithGoogle('https://www.googleapis.com/auth/drive')
          .then(result => {
      ...
      login(token: string, code?: string, theme?: string, queryParams?: Params): void {
        loginSubscription?.unsubscribe();
        patchState(store, {
          user: undefined,
          menus: undefined,
      ...
      loginSuccess(token: Token, queryParams?: Params, redirect = false): void {
        patchState(store, {
