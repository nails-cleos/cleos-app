...
import * as fromActionsLogin from '../user.actions';
...
export class UserEffects {
  ...
  @Effect()
  getAll$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.GET_ALL)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
      return this.userService.getAll(payload.active, payload.direction, payload.page).pipe(
        switchMap((response: any) => {
          return of(new fromActionsLogin.UserSuccess(response));
        }),
        catchError((err: HttpErrorResponse) => of(new fromActionsLogin.UserFailure({error: err.error})))
      );
    })
  );

  @Effect()
  findOne$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.FIND_USER)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
      return this.userService.getById(payload).pipe(
        switchMap((response: any) => {
          return of(new fromActionsLogin.UserSelected({user: response}));
        }),
        catchError((err: HttpErrorResponse) => of(new fromActionsLogin.UserFailure({error: err.error})))
      );
    })
  ...
  @Effect()
  findMe$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.FIND_ME)).pipe(
    map((action: any) => action.payload),
    switchMap(() => {
      return this.userService.getMe().pipe(
        switchMap((response: any) => {
          return of(new fromActionsLogin.UserSelected({user: response, profile: true}));
        }),
        catchError((err: HttpErrorResponse) => of(new fromActionsLogin.UserFailure({error: err.error})))
      );
    })
  ...
  @Effect()
  save$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.SAVE_USER)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
      switch (payload.role) {
        case Role.Customer:
          return this.userService.addCustomer(payload.user).pipe(
            switchMap((response: any) => {
              ...
              return of(new fromActionsLogin.UserSaveSuccess({message}));
            }),
            catchError((err: HttpErrorResponse) => of(new fromActionsLogin.UserFailure({error: err.error})))
          );
  ...
  @Effect()
  update$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.UPDATE_USER)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
  ...
  @Effect()
  delete$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.USER_DELETE)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
  ...
  @Effect()
  resend$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.RESEND_USER_TOKEN)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
  ...
  @Effect()
  changePassword$ = this.actions$.pipe(ofType(fromActionsLogin.UserActionTypes.CHANGE_PASSWORD)).pipe(
    map((action: any) => action.payload),
    switchMap((payload: any) => {
      return this.userService.changePassword(payload.oldPassword, payload.password).pipe(
        switchMap(() => {
          ...
          return of(new fromActionsLogin.ChangePasswordSuccess({message}));
        }),
        catchError((err: HttpErrorResponse) => of(new fromActionsLogin.UserFailure({error: err.error})))
      );
    })
  ...
  dataSuccess$ = this.actions$.pipe(
    ofType(fromActionsLogin.UserActionTypes.USER_SUCCESS)
  );
  ...
  changePasswordSuccess$ = this.actions$.pipe(
    ofType(fromActionsLogin.UserActionTypes.CHANGE_PASSWORD_SUCCESS)
  );
