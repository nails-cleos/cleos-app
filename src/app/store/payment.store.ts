  ...
  withMethods((
    ...
    return {
      ...
      recreate(id: string, paymentType: string): void {
        ...
        recreateSubscription = paymentService.recreate(id, paymentType).subscribe({
          next: () => patchState(store, {
            response: { message: translateService.instant('PAYMENT.RECREATE') },
            isLoading: false,
          }),
          error: patchError,
        });
      ...
      create(
        ...
        createSubscription?.unsubscribe();
        patchState(store, { response: undefined, subErrors: undefined, isLoading: true });

        createSubscription = paymentService.add(id, path, status, paymentStatus).subscribe({
          next: (response) => {
            ...
            switch (response.status) {
              case 'approved':
                ...
                break;
              case 'pending':
                message = translateService.instant('COMMON.PAYMENT.PENDING');
                break;
              default:
                message = translateService.instant('ME.PAYMENT.ERROR', { reason: response.message });
                subErrors = [{ message }];
                toastType = 'error';
                break;
            }
            patchState(store, { response: { message, path, toastType, redirect: path }, subErrors, isLoading: false });
          },
          error: patchError,
        });
      },
      ...
      notify(
        ...
        notifySubscription = paymentService.notifyPayment(id, path, resourceId, preferenceId, paymentType).subscribe({
          next: (response) => {
            let subErrors;
            let message;
            let toastType: ToastType = 'success';
            ...
            switch (response.status) {
              ...
              case 'pending':
                ...
                break;
              default:
                message = translateService.instant('COMMON.PAYMENT.ERROR', { reason: response.message });
                subErrors = [{ message }];
                toastType = 'error';
                break;
            }
            patchState(store, { response: { message, toastType, reload }, subErrors, isLoading: false });
          },
          error: patchError,
        });
      },
      ...
      updateById(id: string, payment: IReservationPayment): void {
        ...
        updateByIdSubscription = paymentService.updatePayment(id, payment).subscribe({
          next: (response) => {
      ...
      adjust(payments: IPaymentRequest[]): void {
        ...
        adjustSubscription = paymentService.adjustPayments(payments).subscribe({
          next: () => patchState(store, {
            response: {
              message: translateService.instant('COMMON.PAYMENT.SUCCESS'),
              reload: true,
      ...
      notComplete(reason: string = 'incomplete'): void {
        const message = translateService.instant('ME.PAYMENT.ERROR', { reason });
