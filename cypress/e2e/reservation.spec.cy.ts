...
devices.forEach(({ name, width, height, breakpoints }) => {
  ...
  describe(`Reservation with ${name}`, () => {
    ...
    it('should create a reservation', () => {
      ...
      cy.wait('@getAdditionalSearch')
        .its('response.statusCode')
        .should('eq', 200);

      // Select additional
      cy.get('mat-list-option').contains('Powder').click({ force: true });
      cy.get('mat-list-option')
        .contains('Powder')
        .closest('mat-list-option')
        .find('input[type="checkbox"]')
        .should('be.checked');
      cy.get('mat-list-option')
        .contains('Biab Removal')
        .closest('mat-list-option')
        .find('input[type="checkbox"]')
        .should('be.checked');
      cy.get('mat-list-option')
        .contains('GelPolish Removal')
        .closest('mat-list-option')
        .find('input[type="checkbox"]')
        .should('be.not.checked');
      cy.get('mat-list-option')
        .contains('Removal from other salon')
        .closest('mat-list-option')
        .find('input[type="checkbox"]')
        .should('be.not.checked');
      ...
      cy.checkAppDialog(
        'Update reservation',
        `Are you sure to change reservation to ${formattedDate}, ${reservationTime}:00?`,
        'Yes',
      );
      ...
      cy.get('.booking-summary').within(() => {
        cy.contains('.summary-item__label', 'Customer')
          .parent()
          .should('contain.text', customerName);
        cy.contains('.summary-item__label', 'Room')
          .parent()
          .should('contain.text', 'Nails Cleos - EUR (€)');
        cy.contains('.summary-item__label', 'Professional')
          .parent()
          .should('contain.text', 'Nails Cleos');
        cy.contains('.summary-item__label', 'Treatment')
          .parent()
          .should('contain.text', 'Biab + Single Color');
        cy.contains('.summary-item__label', 'Additional')
          .parent()
          .should('contain.text', 'Biab Removal')
          .and('contain.text', 'Powder');
        cy.contains('.summary-item__label', 'Date & time')
          .parent()
          .should('contain.text', reservationTime);
        ...
        cy.contains('.price-row--total', 'Current total').should(
          'contain.text',
          '€ 105.00',
        );
      });
      ...
      cy.get('.support-card')
        ...
        .within(() => {
          ...
          cy.contains('.price-row', 'Powder').should('contain.text', '€ 15.00');
          cy.contains('.price-row--total', 'Total').should(
            'contain.text',
            '€ 105.00',
          );
        });
