...
const getAddTreatmentInput = () =>
  cy.get('#add-treatment').then(($treatment) => {
    cy.wrap($treatment).scrollIntoView();
    return cy.wrap($treatment).should('exist').and('not.be.disabled');
  });
...
devices.forEach(({ name, width, height, breakpoints }) => {
  describe(`Treatments with ${name}`, () => {
    ...
    it('should create a new treatment', () => {
      ...
      cy.formControlType(
        'description',
        `${treatmentGroupName} Description`,
        'textarea',
      );
      ...
      treatments.forEach((treatment) => {
        ...
        cy.get('mat-tab-body[aria-hidden="false"]')
          .find('#treatment-description')
          .then(($treatment) => {
            cy.wrap($treatment).scrollIntoView();
            cy.wrap($treatment).should('be.visible');
            cy.wrap($treatment).clear();
            cy.wrap($treatment).type(`${treatment.name} Description`);
          });

        cy.get('mat-tab-body[aria-hidden="false"]')
          .find('#treatment-duration')
          .click({ force: true });
      ...
      cy.wait('@saveTreatment').then((treatmentData) => {
        ...
        expect(body.colorIds).to.have.deep.members([
          '03336843-10cd-4139-888f-77c9499fde6b',
        ]);
        treatments.forEach((treatment, index) => {
          expect(body.treatments[index].primary).to.eq(index === 0);
          expect(body.treatments[index].name).to.eq(treatment.name);
          expect(body.treatments[index].description).to.eq(
            `${treatment.name} Description`,
          );
          expect(body.treatments[index].time).to.eq(
            `${zeroPad(treatment.hour)}:${zeroPad(treatment.minute)}`,
          );
    ...
    it('should edit a treatment', () => {
      ...
      cy.get('@selectedTreatment').then((treatment: any) => {
        ...
        cy.get('[data-cy="priceFrom-input"]').should(
          'have.value',
          treatment.priceFrom,
        );
        ...
        treatment.treatments.forEach((treatment: any) => {
          ...
          cy.get('mat-tab-body[aria-hidden="false"]')
            .find('#treatment-name')
            .should('have.value', treatment.name);
          cy.get('mat-tab-body[aria-hidden="false"]')
            .find('#treatment-description')
        ...
        cy.get('mat-tab-body[aria-hidden="false"]')
          ...
          .then(($treatment) => {
            ...
            cy.wrap($treatment).should('be.visible');
            cy.wrap($treatment).clear();
            cy.wrap($treatment).type(`${newTreatment.name} Description`);
          });
        ...
        cy.wait('@updateTreatment').then((treatmentData) => {
          ...
          expect(body.description).to.eq(`${treatmentGroupName} Description`);
          expect(body.priceFrom).to.eq(priceFrom);
          const expectedColorIds = treatment.colors
            .filter((color: { name: string }) => color.name !== 'Jolie ')
            ...
            ...treatment.treatments
              .filter(
                (treatment: { name: string }) =>
                  treatment.name !== 'Biab Removal Basic ',
              )
              .map(
                (
                  treatmentData: { order: number; duration: number },
                  index: number,
                ) => ({
                  ...treatmentData,
                  order: index,
