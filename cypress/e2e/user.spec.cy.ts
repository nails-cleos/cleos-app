...
const mapRole = new Map([
  [
    'Customer',
    { url: 'customers', displayName: 'Customer 1', otherButtons: [] },
  ],
  [
    'Professional',
    {
      url: 'professionals',
      displayName: 'Nails Cleos',
      otherButtons: ['merge'],
    },
  ],
  [
...
const waitForEmptyUsersState = () => {
  ...
  cy.contains('.no-content', 'No users', { timeout: 15000 }).should(
    'be.visible',
  );
};
...
const waitForUserRow = (displayName: string) => {
  cy.get('.app-table-shell').should('be.visible');
  cy.contains('tr.app-table-master-row', displayName, {
    timeout: 15000,
  }).should('exist');
};

const setColorInput = (dataCy: string, value: string) => {
  cy.get(`[data-cy="${dataCy}"]`, { timeout: 15000 }).then(($data) => {
    cy.wrap($data).scrollIntoView();
    cy.wrap($data).should('exist');
    cy.wrap($data).and('not.be.disabled');
    cy.wrap($data).clear({ force: true });
    cy.wrap($data).type(value, { force: true });
  });
};
    ...
    mapRole.forEach((value, role) => {
      ...
      it(`should edit a ${role}`, () => {
        ...
        cy.get('@selectedUser').then((user: any) => {
          ...
          cy.get('[data-cy="displayName-input"]').should(
            'have.value',
            user.displayName,
          );
          cy.get('#select-lang').contains('English');
          cy.get('input[formControlName="numberControl"]').should(
            'have.value',
            user.phone,
          );
          ...
          cy.get('[data-cy="dob-picker"]').clear();
          cy.get('[data-cy="dob-picker"]').click({ force: true });
          cy.get('td[data-mat-row="1"][data-mat-col="1"]')
            .find('button')
            .click({ force: true });
